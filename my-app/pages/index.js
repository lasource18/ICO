...
export default function Home() {
  ...
  // waletConnected keeps track of whether the user's wallet is connected
  const [walletConnected, setWalletConnected] = useState(false)
  // loading is set to true when we are waiting for a transaction to get mined
  const [loading, setLoading] = useState(false)
  // tokensToBeClaimed keeps track of the number of tokens that can be claimed
  // based on on the Crypto Dev NFT's held by the user for which they haven't claimed the tokens
  ...
  const getTokensToBeClaimed = async () => {
    try {
      ...
      // balance is a Big Number and tus we would compare it with Big Number`zero`
      if(balance === zero)
        setTokensToBeClaimed(zero)
      else {
        // For all NFTs, check if the tokens have already been claimed
        // Only increase the amount if the tokens hae not been claimed
        // for an NFT (for a given toeknId)
      ...
      // tokenToBeClaimed has been initialized to a Big Number, thus we would convert amount
      // to a Big Number then set its value
  ...
  const getBalanceOfCryptoDevTokens = async () => {
    try {
      // Get the provider from web3Modal, which in our case is MetaMask
      // No need for the Signer here, we are only reading state from the blockchain
      const provider = await getProviderOrSigner()
      const tokenContract = new Contract(TOKEN_CONTRACT_ADDRESS, TOKEN_CONTRACT_ABI, provider)
      // we will get the signer now to to extract the address of the currently connected MetaMask account
      const signer = await getProviderOrSigner(true)
      // Get the address associated to the signer which is connected to  MetaMask
      const address = await signer.getAddress()
  ...
  const mintCryptoDevToken = async (amount) => {
    try {
      ...
      const tx = await tokenContract.mint(amount, {
        // value means the cost of 1 crypto dev token is "0.001" eth
  ...
  /**
   * withdrawCoins: withdraws ether and tokens by ca;lling
   ...
   */
    ...
    */
    ...
    useEffect(() => {
      if(!walletConnected) {
        // Assign Web3Modal class to the reference object by setting its current value
    ...
    /*
     ...
     const renderButton = () => {
      ...
      // If tokens to be claimed are > 0, Returns a claim button
        ...
        <div className={styles.main}>
          <div>
            ...
            {walletConnected ? (
              <div>
                <div className={styles.description}>
                  {/* Format Ether hels us in converting a BigNumber to a string */}
